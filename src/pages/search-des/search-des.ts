...
/**
 * Generated class for the SearchDesPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-search-des',
  templateUrl: 'search-des.html',
})
export class SearchDesPage {
  ...
  getSearchList(){
    var that = this;
    this.httpServers.requestData(true,'/trip/?query=Destination:'+that.keywords, that.tools.getUserInfo(), (data) => {
      // console.log(data);
      if(data){
  ...
      if (array.indexOf(array[i].Destination) == i) 
          n.push(array[i]);
    }
      console.log(n);
  ...
  uniqueLoad(arr) {
    ...
    var data = new Array();
      for(var i in arr){
      //该元素在tmp内部不存在才允许追加
        if(tmp.indexOf(arr[i].Destination)==-1){
          tmp.push(arr[i].Destination);
          data.push(arr[i]);
        }
      }
      return data;
